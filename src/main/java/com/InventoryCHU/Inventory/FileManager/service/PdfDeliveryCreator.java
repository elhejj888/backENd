...
import java.util.stream.Collectors;
    ...
    @Service
    public class PdfDeliveryCreator {
        ...
        public PdfDeliveryCreator(String pdfName){
            this.pdfName=pdfName;
        }
        public PdfDeliveryCreator(){}
        ...
        public void createProduct(List<DeliveryDetails> productList) {
            float threecol=190f;
            float fullwidth[]={threecol*3};
            Table threeColTable2=new Table(threeColumnWidth);
            float totalSum=getTotalSum(productList);
            for (DeliveryDetails product:productList)
            {
                ...
                threeColTable2.addCell(new Cell().add(String.valueOf(product.delivered)).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER)).setMarginRight(15f);
        ...
        public float getTotalSum(List<DeliveryDetails> productList) {
            return  (float)productList.stream().mapToLong((p)-> (long) (p.delivered)).sum();
        }

        public List<DeliveryDetails> getDummyProductList()
        {
            List<DeliveryDetails> productList=new ArrayList<>();
            productList.add(new DeliveryDetails("Apple Mac Mini",2,1));
            productList.add(new DeliveryDetails("Ecran Dell",4,2));
            productList.add(new DeliveryDetails("Clavier",2,2));
            productList.add(new DeliveryDetails("Souris",3,2));
            productList.add(new DeliveryDetails("Serveur",5,1));
            productList.add(new DeliveryDetails("Bureau",2,0));
            return productList;
        }
        ...
        public void createTableHeader(ProductTableHeader productTableHeader) {
            ...
            threeColTable1.addCell(new Cell().add("Quantité").setBold().setFontColor(Color.WHITE).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER)).setMarginRight(15f);
            threeColTable1.addCell(new Cell().add("Quantité Livrée").setBold().setFontColor(Color.WHITE).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER)).setMarginRight(15f);
...
//            oneColTable1.addCell(getCell10fLeft(addressDetails.getBillingAddress(),false));
        ...
        public List<DeliveryDetails> modifyProductList(List<DeliveryDetails> productList) {
            Map<String,DeliveryDetails> map=new HashMap<>();
            productList.forEach((i)->{
                if(map.containsKey(i.itemType))
                {
                    i.setQuantity(map.getOrDefault(i.itemType,null).getQuantity()+i.getQuantity());
                    i.setDelivered(map.getOrDefault(i.itemType,null).getDelivered()+i.getDelivered());
